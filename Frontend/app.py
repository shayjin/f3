...
API_BASE = os.getenv("API_BASE_URL", "http://35.163.127.136:8000")
...
def fetch_tab_data(key, query_text, force_refresh=False):
    if force_refresh or st.session_state.tab_data[key] is None:
        with st.spinner(f"AI is retrieving {key}..."):
            try:
                r = requests.post(f"{API_BASE}/ask", json={"query": query_text}, timeout=120)
                if r.status_code == 200:
    ...
    if submit_update:
        ...
        if not (clinical_note or file_content or new_cond or new_med or new_lab):
            ...
            with st.spinner("AI is reading and updating your records..."):
                try:
                    r = requests.post(f"{API_BASE}/ask", json={"query": prompt}, timeout=120)
                    if r.status_code == 200:
    ...
    with st.spinner("🤔Thinking..."):
        try:
            ...
            if response.status_code != 200:
                ...
                answer_text = data.get("answer", "No response from server")
                ...
                if accuracy is not None:
                    ...
                    <span style="
                        ...
                        margin-top:5px;
                    ">
                        Confidence: {label} ({accuracy:.2f})
                    </span>
                    """

                    answer_text += f"<br>{confidence_html}"
            ...
            answer = "Request timed out after 120 seconds."
        except Exception as e:
