...
4. **Configure the service**:
   ...
   **CRITICAL - Override Build Settings**:
   ...
   - Set **Build Command** to: `pip install -r Backend/requirements.txt`
     - **Note**: Use `Backend/requirements.txt` (optimized, smaller image) instead of root `requirements.txt`
   - Set **Start Command** to: `python Backend/main.py`
...
If you get an error that the image is too big:

1. **Use Backend/requirements.txt**: This excludes frontend dependencies (streamlit) and unused ML packages
2. **`.dockerignore` file**: Already created to exclude unnecessary files from the image
3. **Removed packages**:
   - `streamlit` (frontend only, ~50MB saved)
   - `sentence-transformers` (not used, ~100MB+ saved)
   - `matplotlib` and `plotly` (not used in backend, ~30MB saved)

**Total savings: ~180MB+**

The `Backend/requirements.txt` file contains only the dependencies needed for the backend API.

### Option B: Render (Recommended Alternative)
...
4. **Configure**:
   ...
   - **Python Version**: 3.12
   - **Build Command**: `pip install -r Backend/requirements.txt` (optimized, smaller image)
...
For a comprehensive list of alternative deployment platforms (Fly.io, Google Cloud Run, DigitalOcean, Vercel, AWS, Azure), see **[BACKEND_DEPLOYMENT_ALTERNATIVES.md](BACKEND_DEPLOYMENT_ALTERNATIVES.md)**
