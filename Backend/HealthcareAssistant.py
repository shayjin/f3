...
class HealthcareAssistant:
    ...
    async def answer_medical_query(self, query: str, patient_id: str | None = None) -> dict:
        try:
            with self.mcp_client:
                ...
                if patient_id:
                    prompt = f"""
                    You are a healthcare assistant. Answer the following medical query for patient {patient_id}:
                    ...
                    ANSWER:
                    <your main answer, clear and concise>

                    FACT_SUMMARY:
                    - List each fact from patient data, conditions, medications, or observations that you relied on
                    - Use bullet points
                    - Only include verifiable information
                    - Do not add opinions or unrelated commentary

                    End your response with FACT_SUMMARY. Always include both sections.
                    """
                else:
                    prompt = f"""
                    You are a healthcare assistant. Answer the following medical query:
                    ...
                    FACT_SUMMARY:
                    - List each fact from available information that you relied on
                    - Use bullet points
                    - Only include verifiable information
                    - Do not add opinions or unrelated commentary
        ...
        # except Exception as e:
            # return {"error": f"I apologize, but I encountered an error: {str(e)}"}
                ...
                answer, fact_summary = self.parse_agent_response(response_text)
                accuracy = similarity_score(answer, fact_summary) if fact_summary else None
    ...
    def parse_agent_response(response_text: str):
        """Extract ANSWER and FACT_SUMMARY from agent output"""
        answer_match = re.search(r"ANSWER:\s*(.*?)\s*(?=FACT_SUMMARY:|$)", response_text, re.DOTALL)
        facts_match = re.search(r"FACT_SUMMARY:\s*(.*)", response_text, re.DOTALL)

        answer = answer_match.group(1).strip() if answer_match else response_text
        fact_summary = facts_match.group(1).strip() if facts_match else ""

        return answer, fact_summary
